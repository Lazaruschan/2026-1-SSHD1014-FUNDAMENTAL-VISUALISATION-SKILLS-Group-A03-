...
BASE="$(cd "$ROOT/../.." && pwd)"
SRC="$BASE/Notes/Fundamental_Visualisation_Skills_MID/Fundamental_Visualisation_Skills_MID"
OUT="$ROOT/slides"
...
for md in "$SRC"/SHDH1014_Fundamental_Visualisation_Skills_MID_week_*.md; do
  ...
  # Skip consultation / presentation weeks (12+)
  if (( 10#$num >= 12 )); then
...
rm -f "$OUT/week-12.html" "$OUT/week-13.html"

echo "Exported self-contained lecture HTML (weeks 1–11) to $OUT"
